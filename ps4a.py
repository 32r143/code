...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for letter in word:
            word_dict[letter] = word_dict.setdefault(letter,0) + 1
        for k in word_dict:
            if k not in hand:
                return False
            elif word_dict[k] > hand[k]:
                return False
        return True
    else:
        return False
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            if not isValidWord(response, hand, wordList):
                # Reject invalid word (print a message followed by a blank line)
                print('Invalid word, please try again.' + '\n')                
            # Otherwise (the word is valid):
            else:
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                points += getWordScore(response, n)
                print(response,str(getWordScore(response, n)),'earned  points.', 'Total:', str(points), 'points')
                print()
                # Update the hand 
